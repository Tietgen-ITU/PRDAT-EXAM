...
let rec eval (e : expr) (cont : cont) (econt : econt) = 
    ...
    | Random (min, max, num) ->
      match (min, max, num) with
      | mn, mx, _ when mn > mx -> Str "min is greater than max"
      | _, _, nm when 0 >= nm -> Str "num is equal to or less than 0"
      | mn, mx, n -> 
        let random = new System.Random();
        let randomNext(m, x) = random.Next(min,max+1)
        let rec loop = function
          | 0 -> econt () 
          | i -> cont (Int (randomNext(mn, mx))) (fun () -> loop (i-1))
...
let exe2 = Every (Write (Prim("*", FromTo(1,10), FromTo(1,10))));

let exe3 = (Every (Write (Prim("*", FromTo(1,10), Seq(Write(CstS("\n")), FromTo(1,10))))));;
