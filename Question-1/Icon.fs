...
let rec eval (e : expr) (cont : cont) (econt : econt) = 
    ...
    | FromToChar (c1, c2) -> 
      ...
      | ch1, ch2 when System.Char.GetNumericValue(c2) - System.Char.GetNumericValue(c1) < 0.0 -> Str "c2 is not greater than c1"
      | ch1, ch2 -> 
...
let printNumbers = Every (Write (numbers));;

let greaterThan10 = Every(Write(Prim("<", CstI (10), numbers)));;

let newline = (Every (Write (Prim("<", numbers, Seq(Write(CstS("\n")), numbers)))));;

let chars = FromToChar('C','L');;

let charsShort = Every(Write(Prim("<", CstS "G", chars)));;